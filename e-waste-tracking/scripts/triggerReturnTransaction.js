...
module.exports = async (callback) => {
    try {
        ...
        const targetEntityType = "manufacturer"; // Example target entity type

        const accounts = await web3.eth.getAccounts();
        const sourceEntityAddress = accounts[2]; // consumer
        const targetEntityAddress = accounts[1]; // manufacturer


        await eWasteTracking.returnTransaction(
            imeiNumber, 
            sourceEntityType,
            targetEntityType,
            sourceEntityAddress,
        ...
        console.log('Return transaction executed successfully');
